...
export function Comment({id, house, userId}) {
    ...
    const listComment = useSelector(({comments}) => comments.listComment);
    ...
    return (
        <>
            <h2>
                ★{listComment && listComment.reduce((total, next) => total + next.star, 0) / listComment.length} ·{" "}
                {listComment && listComment.length} Đánh giá
            </h2>
                        ...
                        listComment.map((item) => (
                            <div className="row" key={item.id}>
                                <div className="containerComment col-12">
                                    ...
                                    <p>{item.content}</p>
                                    {/* Check userId vs item.user.id ? */}
                                    {userId === item.user.id && (
                                        <div className="centerButtonDeleteComment">
                                            <button
                                                className="buttonDeleteComment btn btn-danger"
                                                style={{float: "right"}}
                                                onClick={() => deleteCommentById(item.id)}
                                            >
                                                del
                                            </button>
                                        </div>
